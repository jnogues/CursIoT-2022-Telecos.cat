// 30_asyncMQTT-BME280.ino
// Aportació de Francesc Moret
...
#define led_blau 16
#define WIFI_SSID     "xxxxxxxxxxxxxxxxxxxxxxxxxxx"
#define WIFI_PASSWORD "xxxxxxxxxxxxxxxxxxxxxxxxx"

#define MQTT_HOST IPAddress(5, 196, 88, 155)
#define MQTT_PORT 1883
 // Cal posar un nou nom a "Canviem" per evitar colisions
#define IDdevice "rebedor"
#define TOPIC1   "/telecos/Canviem/rebedor/uT"         //Per publicar dades
#define TOPIC2   "/telecos/Canviem/rebedor/boot"       //Per publicar dades
#define TOPIC3   "/telecos/Canviem/rebedor/outputs"    //Per publicar dades
#define TOPIC4   "/telecos/Canviem/rebedor/a0"         //Per publicar dades
#define TOPIC5   "/telecos/Canviem/rebedor/inputs"     //Per publicar dades
#define TOPIC6   "/telecos/Canviem/rebedor/state"      //Per publicar dades
#define TOPIC7   "/telecos/Canviem/rebedor/BME280t"    //Per publicar dades Temperatura
#define TOPIC8   "/telecos/Canviem/rebedor/BME280p"    //Per publicar dades Presio
#define TOPIC9   "/telecos/Canviem/rebedor/BME280h"    //Per publicar dades Humitat
#define TOPIC10  "/telecos/Canviem/rebedor/commands"   //Per rebre comandaments
#define TOPIC11  "/telecos/Canviem/rebedor/pwm0"       //Per rebre comandaments
...
void setup() {
  ...
  //BME280
  Serial.println("[BMP ] BME280 test");
  //if (!BME280.init(0x77)) {
  if (!BME280.init(0x76)) {
    Serial.println("[BMP ] Device not connected or broken!");
  }
  ...
  taskScheduler.setInterval(11000L, publishBME280);        //tasca 6
}
...
void publishState()
{
   ...
   jsonDoc["BMPt"]  = temperatureBMP;
   jsonDoc["BMPp"]  = pressureBMP;
   jsonDoc["BMPh"]  = humidityBMP;
...
void publishBME280()
{
  ...
  Serial.print("[BMPt] T: ");
  temperatureBMP = BME280.getTemperature() * 100;
  Serial.println(temperatureBMP);
  //get and print presio
  Serial.print("[BMPp] P: ");
  pressureBMP = BME280.getPressure()/100;
  Serial.println(pressureBMP);
  //get and print humitats
  Serial.print("[BMPh] H: ");
  humidityBMP = BME280.getHumidity();
  Serial.println(humidityBMP);
  //Publicar temperatura,presio i humitat 
  PublishMqtt(TOPIC7, temperatureBMP);
  PublishMqtt(TOPIC8, pressureBMP);
  PublishMqtt(TOPIC9, humidityBMP);
}
...
void onMqttConnect(bool sessionPresent) {
  ...
  //mqttClient.publish("/telecos/Canviem/test", 0, true, "test 1");
  //Serial.println("[MQTT] Publishing at QoS 0");
  //uint16_t packetIdPub1 = mqttClient.publish("/telecos/Canviem/test", 0, true, "test 2");
  //Serial.print("[MQTT] Publishing at QoS 0, packetId: ");
  //Serial.println(packetIdPub1);
  //uint16_t packetIdPub2 = mqttClient.publish("/telecos/Canviem/test", 0, true, "test 3");
  //Serial.print("[MQTT] Publishing at QoS 0, packetId: ");
